...
# Add random step ........................................................... O
# Make a batch generator .................................................... O
# Make data augmenter basen on img modification.............................. O
# Complete x y batch maker................................................... O
# Make function can plot a random window batch............................... O
# Write all documentation in english ........................................ O
# Make a good error system instead an "sys.exit()" error methot bases........ O
# Solve rgb and gray imagese color sistem for generator ..................... O
# Define a output type either one hot encoder or sparse categorical ......... O
# Added warning if a cant load a fram from video ............................ O
# Adde gray color mode ...................................................... O
# solove problems with color transformation, wee need just rgb or gray ...... O


class Vidcg(Sequence):
    ''' Make a data generator to use with a tf.keras.model

    Make a frame sequence "Data generator" over a collection of videos
    ...
    def __init__(self,
        ...
        self.videos_all_paths = self.videos_full_paths()
    ...
    def check(self):
        ...
        if self.color_mode not in ['rgb', 'gray']:
            raise Exception('The color_mode parameter are incorrect')
    ...
    def videos_full_paths(self):
        '''
        make a list of all video system paths and check if a data_path is vaild
        ...
        for folder in self.dirs:
            for video_name in os.listdir(os.path.join(self.data_path, folder)):
                video_absolute_path = os.path.join(self.data_path, folder,
                                                   video_name)
                videos_full_paths.append(video_absolute_path)
        return videos_full_paths
    ...
    def build_all_examples(self):
        ...
        path_to_video :
            string of full path to the video file

        name_caterie :
            str of categorie name
    ...
    def add_random_steeps_examples(self, random_steps=None):
        '''
        Make more examples with random steps augmentation metoth

        Augmented data metoth bases on in random walker over a given (or not)
    ...
    def frames_loader(self, video_path, video_frames, chek=False):
        ...
        for i in video_frames:
            cap.set(cv2.CAP_PROP_POS_FRAMES, i)  # set frame in position i
            # NOTE: remember that cv2.CAP_PROP_POS_FRAMES is a 0 based index

            ret, frame = cap.read()

            # check if a frame is returned
            if not ret:
                raise Exception("Can't read a frame number {}, plz check is "
                      "all correct, terminating proces".format(i))
    ...
    def make_Xy_batch(self, batch):
        ...
        for example in batch:
            category = example['category']  # by name
            category = self.categories_by_name[category]
